# ==============================================================================
# 脚本名称: 03.GTEx_Brain_res.R
# 功能描述: 系统性药物全基因组孟德尔随机化确定脑卒中的治疗靶点 - GTEx_Brain数据分析
# 作者信息: Hongqi Wang
...
###  读入脑组织训练集的阳性结果
gene <- fread(paste0(out_home,"/1.PsychENCODE_res/positive_MR_res.csv")) %>% pull(id.exposure)

### 基因的 eQTL 数据
GTEx_Brain_eqtl <- GTEx_Brain_eqtl %>% filter(exposure %in% gene)

### 循环对数据进行循环连锁不平衡分析
GTEx_Brain_eqtl_clump <- map_df(unique(GTEx_Brain_eqtl$exposure), function(x) {
  source("/Pub/Users/fuxj/mylib/ieugwasr-master/R/ld_clump.R")
  data <- GTEx_Brain_eqtl %>%
    filter(exposure == x) %>%
...
### 判断工具变量是否与脑卒中相关
intersect(GTEx_Brain_eqtl_clump$SNP, stroke_PhenoScanner_SNP)

### 没有 IV 与脑卒中相关，保存结果
save(GTEx_Brain_eqtl_clump, file = paste0(out_dir,"GTEx_Brain_eqtl_clump.RData"))

### 循环进行 MR 分析
all_MR_res <- map_df(unique(GTEx_Brain_eqtl_clump$exposure), function(x) {
  # 将某基因作为暴露因素
  exposure_dat <- GTEx_Brain_eqtl_clump %>% filter(exposure == x)
  # 暴露因素对应的结局数据
...
# 保存结果
write.csv(all_MR_res, paste0(out_dir,"GTEx_Brain_MR_res.csv"), row.names = F)
