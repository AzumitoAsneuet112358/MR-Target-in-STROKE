# ==============================================================================
# 脚本名称: 02.eQTLGene_res.R
# 功能描述: 系统性药物全基因组孟德尔随机化确定脑卒中的治疗靶点 - eQTLGen数据分析
# 作者信息: Hongqi Wang
...
### 循环对数据进行循环连锁不平衡分析
eQTLGene_eqtl_clump <- map_df(unique(eQTLGene_eqtl$exposure), function(x) {
...
### 判断工具变量是否与脑卒中相关
intersect(eQTLGene_eqtl_clump$SNP, stroke_PhenoScanner_SNP)

### 有一个IV与脑卒中相关，保存结果
eQTLGene_eqtl_clump <- eQTLGene_eqtl_clump %>% filter(SNP != "rs62046395")
save(eQTLGene_eqtl_clump, file = paste0(out_dir,"eQTLGene_eqtl_clump.RData"))

### 循环进行 MR 分析
all_MR_res <- map_df(unique(eQTLGene_eqtl_clump$exposure), function(x) {
    # 将某基因作为暴露因素
    exposure_dat <- eQTLGene_eqtl_clump %>% filter(exposure == x)
    # 暴露因素对应的结局数据
...
# 保存结果
write.csv(all_MR_res, paste0(out_dir,"eQTLGene_MR_res.csv"), row.names = F)
...
## 保存结果
pdf(file = paste0(out_dir, "/forestplot.pdf"), w = 9, h = 35)
print(p)
dev.off()
